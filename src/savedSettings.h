...
saved_settings_t default_settings{
    ...
    .struct_version = 10, // Increment this if the structure of the settings changes in a non-backwards compatible way (e.g. changing types of existing fields, removing fields, changing the order of fields, etc) so that we can detect when the saved settings are no longer valid and need to be reset to defaults
...
void load_settings()
{
    ...
    Log.infoln("| SETTINGS: autostart_on: %T, autostart_delay: %lu ms, stats_loging_enabled: %T, stats_log_interval: %lu ms, log_level: %s (%d)", saved_settings.autostart_on, saved_settings.autostart_delay, saved_settings.stats_loging_enabled, saved_settings.stats_log_interval, logLevelToString(saved_settings.log_level).c_str(), saved_settings.log_level);
}
